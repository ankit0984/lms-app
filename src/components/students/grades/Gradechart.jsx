...
//
//     const selected = courseData[selectedCourse]
//     const averageGrade =
//         selected.data.reduce((acc, cur) => acc + cur.grade, 0) / selected.data.length
//
//     return (
//         <Card className="@container/card">
//             <CardHeader className="relative space-y-2">
//                 <div className="flex flex-col lg:justify-between gap-2 sm:flex-row sm:items-center sm:justify-between">
//                     <div>
//                         <CardTitle>Grade Progress</CardTitle>
//                         <CardDescription>Performance across assignments</CardDescription>
//                     </div>
//
...
// New hierarchical structure: departments > semesters > courses
const courseData = {
    CSE: {
        "Sem 1": {
            CS101: {
...
export function GradeProgressChart() {
    ...
    const [selectedSemester, setSelectedSemester] = React.useState("Sem 1")
    const [selectedCourse, setSelectedCourse] = React.useState("CS101")

    const semesters = Object.keys(courseData[selectedDept] || {})
    const courses = Object.keys(courseData[selectedDept]?.[selectedSemester] || {})
    const selected =
        courseData[selectedDept]?.[selectedSemester]?.[selectedCourse]

    React.useEffect(() => {
        // Auto-select first course if current one doesn't exist
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-3 items-center">
                        ...
                        <Select value={selectedDept} onValueChange={(val) => {
                            setSelectedDept(val)
                            setSelectedSemester(Object.keys(courseData[val])[0])
                        }}>
